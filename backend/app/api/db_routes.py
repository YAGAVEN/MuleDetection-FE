...
async def get_analytics(timeRange: str = "7days", metric: str = "transaction_count"):
    """Get analytics data for Chronos visualization"""
    try:
        # In a real scenario, this would query the DB for actual analytics.
        # For now, we return structured mock data to satisfy the frontend.
        from datetime import datetime, timedelta
        import random
        
        days = 7
        if timeRange == "1day": days = 1
        elif timeRange == "30days": days = 30
        elif timeRange == "90days": days = 90
        
        dates = [(datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(days-1, -1, -1)]
        
        transactions = [random.randint(400, 800) for _ in range(days)]
        suspiciousCount = [random.randint(10, 50) for _ in range(days)]
        ...
        return {
            ...
            "patterns": {
                "hourly": {
                    ...
                    "transactions": [random.randint(10, 150) for _ in range(24)]
                },
                "riskDistribution": {
